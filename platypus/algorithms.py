...
from .core import Algorithm, Problem, ParetoDominance, AttributeDominance,\
...
from .operators import TournamentSelector, RandomGenerator,\
    ...
    GAOperator, SBX, PM, UM, PCX, UNDX, SPX, Multimethod, MutateIMGAMOHiper
...
class IMGAMO(Algorithm):
    def __init__(self, problem,
                 ...
                 dominance=ParetoDominance(),
                 **kwargs):
        super(IMGAMO, self).__init__(problem, **kwargs)
        self.population_size = population_size
        self.exchange_iter = exchange_iter
        self.change_iter = change_iter
        self.clone_number = clone_number
        self.sup_level_f = sup_level_f
        self.sup_level_x = sup_level_x
        self.generator = generator
        self.mutate = mutate
        self.exchange_repair = exchange_repair
        self.dominance = dominance
        self.front = None
        self.iterate_num = 0
        self.local_nfe = [0.0 for _ in range(problem.nobjs)]
        self.time_s = 0
        self.time_e = 0
    ...
    def step(self):
        if self.nfe == 0:
            ...
            self.iterate()
            self.iterate_num += 1
            self.result = self.front
            self.time_e = time.process_time()

    def initialize(self):
        self.time_s = time.process_time()
        self.populations = [[self.generator.generate(self.problem) for i in range(self.population_size)]
                            for j in range(self.problem.nobjs)]
        self.patterns = self._assigning_gens(self.problem.nvars, self.problem.nobjs)
        for p in range(self.problem.nobjs):
            self.evaluate_all(self.populations[p])

    def iterate(self):
        for p in range(self.problem.nobjs):
            # clonal selection
            self._clonal_selection(p)
        for p in range(self.problem.nobjs):
            self.evaluate_all(self.populations[p])
        self.nfe += max(self.local_nfe)

        for p in range(self.problem.nobjs):
            # get not dominance
            not_dominated = self._get_not_dominated(p)
            # add to result
            self._add_to_front(not_dominated)
        for p in range(self.problem.nobjs):
            # suppresion
            self._suppression(p)

        # exchange gens
        if self.iterate_num % self.exchange_iter == 0:
            best_vars = []
            for p in range(self.problem.nobjs):
                best_vars.append(self._get_best_vars(p))
            for p in range(self.problem.nobjs):
                self._update_gens(p, best_vars)
            for p in range(self.problem.nobjs):
                if self.exchange_repair is not None:
                    self.exchange_repair(self.populations[p])
                self.evaluate_all(self.populations[p])

        # change patterns
        if self.iterate_num % self.change_iter == 0:
            self.patterns = self._assigning_gens(self.problem.nvars, self.problem.nobjs)

    def _assigning_gens(self, nvars, nobjs):
        temp = np.arange(nvars)
        random.shuffle(temp)
        q, r = divmod(nvars, nobjs)
        if r == 0:
            pat = [temp[i::nobjs] for i in range(nobjs)]
        else:
            temp_pat = [temp[i:-r:nobjs] for i in range(nobjs)]
            pat = copy.deepcopy(temp_pat)
            for i in range(1, r + 1):
                ran = random.randint(0, nobjs - 1)
                pat[ran] = np.append(temp_pat[ran], temp[-i])
        return pat

    def _clonal_selection(self, p):
        self.local_nfe[p] = 0
        temp_eval = np.array([self.populations[p][i].objectives[p] for i in range(self.population_size)])
        arg_sort = np.argsort(temp_eval)
        if self.problem.directions[p] == Problem.MAXIMIZE:
            arg_sort = arg_sort[::-1]
        clone_num = self.clone_number
        for i in arg_sort:
            mutate_pop = [self.mutate.mutate(copy.deepcopy(self.populations[p][i]), self.patterns[p])
                          for _ in range(clone_num)]
            mutate_eval = np.array([self.problem.evaluate_one(clone, p) for clone in mutate_pop])
            self.local_nfe[p] += clone_num
            best_clone = None
            if self.problem.directions[p] == Problem.MAXIMIZE:
                arg_best = np.argmax(mutate_eval)
                if mutate_eval[arg_best] > self.populations[p][i].objectives[p]:
                    best_clone = mutate_pop[arg_best]
            else:
                arg_best = np.argmin(mutate_eval)
                if mutate_eval[arg_best] < self.populations[p][i].objectives[p]:
                    best_clone = mutate_pop[arg_best]
            if best_clone is not None:
                self.populations[p][i].variables[:] = best_clone.variables[:]
                self.populations[p][i].objectives[p] = mutate_eval[arg_best]
                self.populations[p][i].evaluated = False
    ...
    def _get_not_dominated(self,p):
        to_remove = []
        for i in range(len(self.populations[p])):
            for j in range(len(self.populations[p])):
                if j not in to_remove and i != j:
                    if self.dominance.compare(self.populations[p][i], self.populations[p][j]) == 1:
                        to_remove.append(i)
                        break
        not_dominated = []
        for i in range(len(self.populations[p])):
            if i not in to_remove:
                not_dominated.append(copy.deepcopy(self.populations[p][i]))

        to_remove = []
        for i in range(len(not_dominated)):
            for j in range(len(not_dominated)):
                if j not in to_remove and i != j:
                    if self._distance(not_dominated[i].objectives, not_dominated[j].objectives) < self.sup_level_f:
                        to_remove.append(i)
                        break
        not_dominated_s = []
        for i in range(len(not_dominated)):
            if i not in to_remove:
                not_dominated_s.append(copy.deepcopy(not_dominated[i]))
        return not_dominated_s

    def _add_to_front(self, not_dominated):
        if self.front is None:
            ...
            for i in range(len(not_dominated)):
                for j in range(len(self.front)):
                    if self.dominance.compare(not_dominated[i], self.front[j]) == 1:
                        to_remove.append(i)
                        break
            temp_ndom = []
            for i in range(len(not_dominated)):
                if i not in to_remove:
                    temp_ndom.append(copy.deepcopy(not_dominated[i]))

            to_remove = []
            for i in range(len(self.front)):
                for j in range(len(temp_ndom)):
                    if self.dominance.compare(self.front[i], temp_ndom[j]) == 1:
                        to_remove.append(i)
                        break
            new_front = []
            for i in range(len(self.front)):
                if i not in to_remove:
                    new_front.append(copy.deepcopy(self.front[i]))

            new_front_s = copy.deepcopy(new_front)
            for i in range(len(temp_ndom)):
                flag = True
                for j in range(len(new_front)):
                    dist = self._distance(temp_ndom[i].objectives, new_front[j].objectives)
                    if dist < self.sup_level_f:
                        flag = False
                        break
                if flag:
                    new_front_s.append(temp_ndom[i])
            self.front = copy.deepcopy(new_front_s)

    def _distance_matrix(self, p):
        n = self.population_size
        dist_mat = np.zeros((n, n))
        for i in range(n):
            vec1 = [self.populations[p][i].variables[k] for k in self.patterns[p]]
            for j in range(i, n):
                vec2 = [self.populations[p][j].variables[k] for k in self.patterns[p]]
                if i == j:
                    dist_mat[i, j] = 0.0
                else:
                    dist_mat[i, j] = self._distance(vec1, vec2)
                    dist_mat[j, i] = dist_mat[i, j]
        return dist_mat

    def _suppression(self, p):
        dist_mat = self._distance_matrix(p)
        computed = []
        for k, row in enumerate(dist_mat):
            if k in computed:
                continue
            similar = [j for j in range(len(row)) if row[j] < self.sup_level_x and j not in computed]
            if len(similar) == 1:
                computed += similar
                continue
            pop_similar = np.array([self.populations[p][j].objectives[p] for j in similar])
            arg_sort = np.argsort(pop_similar)
            if self.problem.directions[p] == Problem.MAXIMIZE:
                arg_sort = arg_sort[::-1]
            new_ind = self.generator.generate(self.problem)
            for k in self.patterns[p]:
                self.populations[p][similar[arg_sort[1]]].variables[k] = new_ind.variables[k]
                self.populations[p][similar[arg_sort[1]]].evaluated = False
            computed += [similar[arg_sort[1]]]
        self.evaluate_all(self.populations[p])

    def _get_best_vars(self, p):
        pop_eval = np.array([self.populations[p][i].objectives[p] for i in range(self.population_size)])
        if self.problem.directions[p] == Problem.MAXIMIZE:
            arg_best = np.argmax(pop_eval)
        else:
            arg_best = np.argmin(pop_eval)
        return copy.deepcopy(self.populations[p][arg_best].variables)

    def _update_gens(self, p, best_vars):
        for k in range(len(self.patterns)):
            if k == p:
                continue
            for j in range(len(self.populations[p])):
                for i in self.patterns[k]:
                    self.populations[p][j].variables[i] = best_vars[k][i]
                    self.populations[p][j].evaluated = False
