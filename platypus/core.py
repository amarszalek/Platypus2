...
class VariatorIMGAMO(object):
    __metaclass__ = ABCMeta

    def __init__(self, arity):
        super(VariatorIMGAMO, self).__init__()
        self.arity = arity

    @abstractmethod
    def evolve(self, parents, pattern):
        raise NotImplementedError("method not implemented")


class MutationIMGAMO(VariatorIMGAMO):
    __metaclass__ = ABCMeta

    def __init__(self):
        super(MutationIMGAMO, self).__init__(1)

    def evolve(self, parents, pattern):
        if hasattr(parents, "__iter__"):
            return list(map(lambda p: self.mutate(p, pattern), parents))
        else:
            return self.mutate(parents, pattern)
