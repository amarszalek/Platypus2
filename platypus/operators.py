...
import math
import random
from .core import PlatypusError, Solution, ParetoDominance, Generator, Selector, Variator, Mutation, EPSILON, VariatorIMGAMO, MutationIMGAMO
...
class MutateIMGAMOUniform(MutationIMGAMO):
    def __init__(self, probability=1.0):
        super(MutateIMGAMOUniform, self).__init__()
        self.probability = probability

    def mutate(self, parent, pattern):
        child = copy.deepcopy(parent)
        problem = child.problem
        probability = self.probability

        if isinstance(probability, int):
            probability /= float(len(pattern))
            for i in pattern:
                if isinstance(problem.types[i], Real):
                    if random.uniform(0.0, 1.0) <= probability:
                        child.variables[i] = random.uniform(problem.types[i].min_value, problem.types[i].max_value)
                        child.evaluated = False
        else:
            i = random.choice(pattern)
            if isinstance(problem.types[i], Real):
                child.variables[i] = random.uniform(problem.types[i].min_value, problem.types[i].max_value)
                child.evaluated = False
        return child
...
class MutateIMGAMOGaussian(MutationIMGAMO):
    def __init__(self, probability=0.1):
        super(MutateIMGAMOGaussian, self).__init__()
        self.probability = probability
    ...
    def mutate(self, parent, pattern):
        ...
        if isinstance(probability, tuple):
            probability_cond = probability[0] / float(len(pattern))
            sigma = probability[1]
            for i in pattern:
                if isinstance(problem.types[i], Real):
                    if random.uniform(0.0, 1.0) <= probability_cond:
                        new_var = random.gauss(child.variables[i], sigma)
                        if problem.types[i].min_value < new_var < problem.types[i].max_value:
                            child.variables[i] = new_var
                            child.evaluated = False
        else:
            i = random.choice(pattern)
            sigma = probability
...
class MutateIMGAMOBound(MutationIMGAMO):
    def __init__(self, probability=1.0):
        super(MutateIMGAMOBound, self).__init__()
        self.probability = probability
            ...
            for i in pattern:
                if isinstance(problem.types[i], Real):
                    if random.uniform(0.0, 1.0) <= probability:
                        ...
                        a = problem.types[i].min_value
                        b = problem.types[i].max_value
                        if ran > 0.5:
                            ran = random.uniform(0.0, 1.0)
                            child.variables[i] = a + (child.variables[i] - a) * ran
                        else:
                            ran = random.uniform(0.0, 1.0)
                            child.variables[i] = (b - child.variables[i]) * ran + child.variables[i]
                        child.evaluated = False
        else:
            i = random.choice(pattern)
            if isinstance(problem.types[i], Real):
...
class MutateIMGAMOHiper(MutationIMGAMO):
    def __init__(self, probability=(0.45,0.45,0.1)):
        super(MutateIMGAMOHiper, self).__init__()
        self.probability = probability
    ...
    def mutate(self, parent, pattern):
        ...
        uniform = MutateIMGAMOUniform(probability=1.0)
        gauss = MutateIMGAMOGaussian(probability=0.1)
        bound = MutateIMGAMOBound(probability=1.0)

        ran = random.uniform(0.0, 1.0)
        if ran < probability[0]:
            child = uniform.mutate(child, pattern)
        elif ran < probability[0] + probability[1]:
            child = gauss.mutate(child, pattern)
        else:
            child = bound.mutate(child, pattern)
        return child
